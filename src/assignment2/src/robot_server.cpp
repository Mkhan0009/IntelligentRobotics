...
class RobotAction
{
    ...
    public:
        ...
        void executeCB(const assignment2::RobotGoalConstPtr &goal)
        {
            // set sleep durations
        ...
        void scanCB(const sensor_msgs::LaserScan::ConstPtr& laser_scan)
        {
            ...
            // we know in advance there are obstacles
            if (result_.obstacle_locations_x.size() < 0 ||
                result_.obstacle_locations_y.size() < 0)
            {
                succeeded_ = false;
            }
        }
