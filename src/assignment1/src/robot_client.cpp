...
{
    ros::init(argc, argv, "test_assignment1");
    ...
    if ( argc < 7 )
    {
        ...
        ROS_INFO(" ");
        ROS_INFO("\texample: assignment1 robot_client 11.37 -0.34 0 0 0 -0.35 0.94");
        ROS_INFO(" ");
    ...
    actionlib::SimpleActionClient<assignment1::RobotAction> ac("robot", true);
    ROS_INFO("Waiting for action server to start.");
    ac.waitForServer(); //will wait for infinite time
    ROS_INFO("Action server started, sending goal.");

    // set the goal from the user input
    assignment1::RobotGoal goal;
    float x = atof(argv[1]);
    float y = atof(argv[2]);
    float z = atof(argv[3]);
